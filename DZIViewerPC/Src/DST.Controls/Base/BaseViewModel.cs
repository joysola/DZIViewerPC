...
namespace DST.Controls.Base
{
    public abstract class BaseViewModel : ViewModelBase
    {
        ...
        private IEnumerable<IDataInput> _hcVerifyControls = new List<IDataInput>();
        /// <summary>
        /// 表示HC控件
        /// </summary>
        internal IEnumerable<IDataInput> HCVerifyControls
        {
            get => _hcVerifyControls;
            set
            {
                if (value != null)
                {
                    var tmp = _hcVerifyControls.ToList(); // 返回一个新list
                    foreach (var newItem in value)
                    {
                        if (_hcVerifyControls.Contains(newItem))
                        {
                            continue;
                        }
                        else
        ...
        /// 校验所有HC控件是否验证成功
        /// </summary>
        public bool IsCheckedOK => HCVerifyControls.VerifyHCCtlsData();
