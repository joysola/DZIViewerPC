...
namespace DST.Common.Helper.ExcelHelper
{
    ...
    public sealed class ExcelHelper
    {
        ...
        /// <returns></returns>
        public static bool WriteExcelFile(string filePath, DataTable dt)
        {
            if(dt == null)
            {
                return false;
            ...
            {
                res = false;
                Logger.Error("写入Excel表格失败：" + ex.Message);
            }
        ...
        public static ResultModelList<T> ReadExcelFile<T>(string filePath, int worksheetIndex = 0, int firstRowIndex = 0) where T : new()
        {
            ...
            if (excelModelAttr == null)
            {
                ...
                Logger.Error("导入Excel故障：" + result.ErrorMessage);
                return result;
            }
