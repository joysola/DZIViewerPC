...
namespace DST.Common.Helper
{
    public class ZipHelper
    {
        ...
        private static async Task<bool> ZipDirectory(string folderToZip, ZipOutputStream zipStream, string parentFolderName, Action<long> zipPerCallback = null)
        {
            ...
            try
            {
                parentFolderName = Path.Combine(parentFolderName, Path.GetFileName(folderToZip) + "/");
            ...
            {
                Logger.Error("生成ZIP文件错误(ZipHelper.ZipDirectory)：" + ex.Message);
                result = false;
            }
            finally
            {
            ...
            foreach (string folder in folders)
            {
                ...
                if (await ZipDirectory(folder, zipStream, parentFolderName, zipPerCallback).ConfigureAwait(false) == false)
                {
                    return false;
                }
            }
        ...
        public static async Task<bool> ZipFile(string fileToZip, string zipedFile, string password)
        {
            ...
            {
                Logger.Error("生成ZIP文件错误(ZipHelper.ZipFile)：" + ex.Message);
                result = false;
            }
            finally
            {
        ...
        public static async Task<bool> UnZipFiles(string zipedFileName, string unZipDirectory, string password)
        {
            ...
            {
                Logger.Error("解压ZIP文件错误(ZipHelper.UnZipFiles)：" + ex.Message);
                return false;
            }
            finally
            {
