...
namespace DST.Common.TscBarcodePrint
{
    /// <summary>
    /// TSC 条码打印API
    /// </summary>
    public class TSCLibApi
    {
        ...
        static TSCLibApi()
        {
            ...
            LoadLibrary($@"{folder}\TscBarcodePrint\{subfolder}\{dllName}");
        }
        /// <summary>
        /// 顯示 DLL 版本號碼
        /// </summary>
        ...
        /// </summary>
        /// <returns></returns>
        public static bool PrintStart()
        {
            bool result = true;
            try
            {
                OpenPort("TSC TTP-244 Pro"); // 打开端口
                Setup("51", "17.2", "5", "15", "0", "2", "0");
                SendCommand("SET TEAR ON"); // The label gap will stop at the tear off position after print.
                ClearBuffer(); // 清除缓存
            }
            catch(Exception e)
            {
                Logger.Error("TSC 条码打印启动异常！" + e.Message);
                result = false;
            }
